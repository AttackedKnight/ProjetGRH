...
angular.module('ParametrageModule').controller('SituationMatrimonialeController', function ($scope, Securite, SweetAlert, Situation) {
    ...
    $scope.add = function (s) {
        ...
        Situation.add(s).success(function () {
            SweetAlert.simpleNotification("success", "Succes", "Situation matrimoniale ajoutée avec succes");
            $scope.findAll();
            $scope.situation = {id: ""};
        }).error(function () {
            SweetAlert.simpleNotification("error", "Erreur", "La Situation matrimoniale n'a pas pu etre ajoutée");
        });
    };
    ...
    $scope.edit = function (item) {
        SweetAlert.attendreTraitement("Traitement en cours", "Veuillez patienter svp !");
        Situation.edit(item).success(function () {
    ...
    $scope.delete = function (item) {
        ...
        Promise.resolve(SweetAlert.confirmerAction("Attention", "Voulez vous vraiement supprimer cet élément ?"))
                .then(function (value) {
                    if (value == true) {
                        SweetAlert.attendreTraitement("Traitement en cours", "Veuillez patienter svp !");
                        Situation.delete(item.id).success(function () {
