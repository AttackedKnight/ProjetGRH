...
if ( !AmCharts.translations[ "export" ][ "en" ] ) {
    ...
    "label.saved.from": "Sauvegarder le: "
	}
}
				...
				handler: {
					...
					text: function( options ) {
						var cfg = _this.deepMerge( {
							...
							top: _this.setup.fabric.height / 10,
							left: _this.setup.fabric.width / 2,
				...
				if ( !cfg.content ) {
					...
					if ( cfg.pageOrigin ) {
						pageContent.push( _this.i18l( "label.saved.from" ) );
                                                var ladate=new Date();
						pageContent.push(ladate.getDate()+"/"+(ladate.getMonth()+1)+"/"+ladate.getFullYear());
                                                pageContent.push(ladate.getHours()+":"+ladate.getMinutes()+":"+ladate.getSeconds());
